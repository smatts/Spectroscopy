...
%
%
%Using this formalism, we now integrate up the Schrödinger equation \ref{eq:2d_schroedinger_interaction}. We have to taken into account that wave functions and operators in eq.  \ref{eq:2d_schroedinger_interaction} are in the interaction picture, while we usually state things as eq. \ref{eq:2d_hamilton_perturbation} in the Schrödinger picture. We thus need to time-evolve the perturbation operator
%\begin{equation}
%  H'_I(t) = U_0^\star(t, t_0) \, H'_S(t) \, U_0(t, t_0)
%\end{equation}
%with the operator for free (= unperturbed) time evolution $U_0$ from time $t_1$ to $t_2$
%\begin{equation}
% U_0(t_2, t_1) = \exp \left(- \frac{i}{\hbar} \hat{H}_0 \, ( t_2- t_1) \right)
...
\includestandalone[width=106mm]{\currfiledir f2tikz}
\caption{
...
\includestandalone[width=106mm]{\currfiledir f3tikz}
\caption{
